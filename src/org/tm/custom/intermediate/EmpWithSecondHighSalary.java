package org.tm.custom.intermediate;
...
public class EmpWithSecondHighSalary {
    public static void main(String[] args) {
        ...
        Optional<Double> secondHigh = employees.stream().map(Employee::getSalary)
                .distinct()
                .sorted(Comparator.reverseOrder())
                .skip(1)
                .findFirst();

        System.out.println("Second High Salary " + secondHigh);

        List<Employee> result = employees.stream()
                .filter(emp -> emp.getSalary() == secondHigh.get())
                .toList();
        ...
        Optional<Employee> secondHighest = employees.stream()
                ...
                .filter(distinctByKey(Employee::getSalary)) // distinct by salary only
                .skip(1)
                .findFirst();
        ...
        List<Employee> emp = employees.stream()
                ...
                .collect(Collectors.collectingAndThen(
                        ...
                        max -> employees.stream()
                                .filter(i ->i.getSalary() == max.get(0))
                                .toList()
                        ));
